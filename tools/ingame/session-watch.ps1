# In-game session helper (foundation step 5, session 12). Run in the background after the server's boot line:
#   pwsh -NoProfile -File tools/ingame/session-watch.ps1
# It watches BepInEx/LogOutput.log for admin marker commands (`.nyar event stop go|d23a|d23b`) and rewrites the dev
# server's events.json through session-events.py; after "[nyar] purge:" followed by "event t-manual wave 2/2" it stops
# the server right after the next "Finished Saving" (D21 setup). Notes go to %TEMP%
yar-session\watcher.log.
...
$go = $a = $b = $restored = $armed = $false; $saves = 0
...
while ($true) {
    ...
    if (-not $armed) {
        ...
        if ($i -ge 0 -and $t.Substring($i) -match 'event t-manual wave 2/2') {
            ...
            Note "armed for D21: t-manual wave 2/2 after the purge; saves so far $saves"
        }
    } else {
