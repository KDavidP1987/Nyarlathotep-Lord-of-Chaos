...
- S-11 · reversible · ConfigFile.Save rewrites the cfg keeping every other key's value and the comments BepInEx writes · fallback: if D15's diff shows more than one changed line, PillarSwitches stops calling ConfigFile.Save itself and only sets the [Pillars] entries' ConfigEntry.Value, leaving the save to BepInEx's own SaveOnConfigSet; the cfg is still written only by BepInEx, and if that too rewrites other lines, the release waits for an owner decision recorded as an amendment of D15
