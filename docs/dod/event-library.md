...
| data inventory (D30) | DataInventory/bad to bad-3 (an entry missing a field), bad-temp (a `temp:` glob with no entry), and new bad-el (event-library.md in dataTables with one of its Design › Data rows' entries removed) | good | empty → "data inventory: no entries" |
...
| 3.3 persistence | D30 `pwsh tools/preflight.ps1` data inventory line, with docs/dod/event-library.md in dataTables | a Design › Data row without a complete entry (fixture DataInventory/bad-el) | a complete inventory | a missing data-inventory.json → failure |
| 4.4 precedence | D16 `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter "FullyQualifiedName~ReadinessTests or FullyQualifiedName~ControlPrecedenceTests"` | a lower cause winning over a higher one, in the column or in the start refusal, or the column's label differing from StartBlocker's reply in any of the 64 combinations | each single cause | 0 cases → failure |
| 6.2 dependencies | in game: D19 `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter FullyQualifiedName~DependencyFailureTests`; release time: D18 `pwsh tools/preflight.ps1 -SelfTest`, which re-runs release-verify's and repo-rollback-drill's failure cases | a failure changing memory or escaping its command; a promote-then-throw fake leaving memory different from the file or replying anything but "write uncertain: file now holds <n> definitions"; a delete whose state.json write fails restoring the definition or keeping the row past the next save; a release-tool failure case printing its success line | healthy fakes; the good cases | 0 cases → failure; an empty registry → failure |
...
| 12.4 failing cases | D31 and D18 in one line: `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter "FullyQualifiedName~ControlCaseTests or FullyQualifiedName~TemplateLibraryTests or FullyQualifiedName~TemplateCommandTests or FullyQualifiedName~AuthoringTests or FullyQualifiedName~AuthoringCapacityTests or FullyQualifiedName~PillarSwitchTests or FullyQualifiedName~ReadinessTests or FullyQualifiedName~LibraryDependencyFailureTests or FullyQualifiedName~CommandArgTests or FullyQualifiedName~ConfigChangedTests or FullyQualifiedName~AuthorizationTests or FullyQualifiedName~ContractDocTests or FullyQualifiedName~ControlPrecedenceTests"; pwsh tools/preflight.ps1 -SelfTest` (the second runs every script check's fixtures and D24's soak selftest) | a control without its fails_when, empty or passes case, or a case named in the ControlCases.cs table missing from its class; a bad or empty fixture passing | complete case sets; good fixtures | a class with no test methods, or a filter running 0 tests → failure; an empty registry → failure |
...
1. **Records and pure logic.**
   - Create docs/audits/event-library.md (rollback base v0.4.0) and docs/features/EVENT_LIBRARY.md with Status, Test plan and an empty Test results, and map event-library to that doc in tools/preflight-checks.json childDocs; add docs/dod/event-library.md to its dataTables with the fixture tools/preflight-fixtures/DataInventory/bad-el (this plan's Design › Data with one row's inventory entry removed), registered there (D30).
   ...
   - Extend Logic/EventAdmin.cs (the trigger and action fields, readiness in EventLines), Logic/CommandArgs.cs (settable fields and shape checks), Logic/DefinitionEditor.cs (Append, Copy, Delete, the reload after an uncertain write), Logic/DataStore.cs (a promote failure reported apart from a .tmp failure), Logic/ActionGateway.cs (CreateEvent, DeleteEvent, SetPillar granted to Admin), Logic/Idempotency.cs (DeleteArming).
   - Tests: TemplateLibraryTests, TemplateCommandTests, AuthoringTests, AuthoringCapacityTests, PillarSwitchTests, ReadinessTests, LibraryDependencyFailureTests (in DependencyFailureTests.Library.cs), ControlCaseTests with its table ControlCases.cs (D31: one row per control naming its bad, good and empty methods), and additions to CommandArgTests, ConfigChangedTests, AuthorizationTests, ContractDocTests and ControlPrecedenceTests, every new method named by D31's convention; LibraryDependencyFailureTests gains the delete whose state.json write fails; FakeStores gains a fake IPillarStore, a state store that fails its next write, and a FakeFileStore mode that promotes the new file and then throws.
   - Run `dotnet test Nyarlathotep/Nyarlathotep.Tests`.
   - Satisfies D1, D2, D4, D5, D6, D7, D8, D9, D10, D11, D12, D13, D14, D16, D19, D27, D31, and the dataTables half of D30.
...
- W5.3 · **Release and records** · items: D28 D29 D30 · steps: 1, 6
...
- **Step 1:**
  ...
  - tools/preflight-checks.json (childDocs, dataTables), tools/preflight-fixtures/DataInventory/bad-el/**.
...
- **Step 3:** tools/soak-report.ps1, tools/soak-report-fixtures/**, tools/preflight.ps1, tools/preflight-checks.json, tools/preflight-fixtures/Paths/bad-soak/**, tools/preflight-fixtures/Paths/bad-undeclared/**, tools/paths-manifest.txt, tools/data-inventory.json, tools/ingame/session-events.py.
...
- S-11 · validated · a dependency contract, gated by D15: setting a [Pillars] ConfigEntry.Value makes BepInEx save the cfg itself (ConfigFile.SaveOnConfigSet, on by default), and after a save that throws, ConfigFile.Reload re-reads the file so memory matches it (D19); that the save keeps every other key's value and the comments BepInEx writes is the half of the contract D15 checks in game, and a D15 diff showing more than the one switched line fails D15 and stops the build for an amendment before step 5 (no alternative writer is preselected: the cfg stays written only by BepInEx, never by a direct file write) · source: BepInEx.Core 6.0.0-be.733 XML docs (SaveOnConfigSet "writes the config to disk every time a value is set"; Reload "reloads the config from disk"); owner accepted Review 4 F6 on 2026-09-26 (plan mode, option A); gate D15
...
| 6 | External dependencies & contracts | Considered | 3/3 | Interfaces › 6.1 D3 D15 D25; 6.2 D19 D18 D8; 6.3 D24 D30 |
| 7 | States & lifecycle | Considered | 3/3 | Design › States › 7.1 D3 D4 D16 D23; 7.2 D8 D12 D14; 7.3 D8 D12 D19 D26 |
