...
- [ ] D8 · **event delete, confirmed** `.nyar event delete <id>` writes nothing and replies "delete <id>? run .nyar event delete <id> confirm within 30 s"; `.nyar event delete <id> confirm` from the same admin within 30 s removes the definition, clears its state.json cooldown row, and replies "event <id> deleted (events.json.bak keeps the previous file)". Logic/Idempotency.cs `DeleteArming` (the PurgeArming pattern) keys the pending delete by admin id and event id. A confirm without a pending delete, after 30 s, from another admin, or for another id replies "no delete pending for <id>"; a delete of a running event replies "event <id> is running; stop it first" and arms nothing, and a confirm when the event started after arming refuses the same way · test: Nyarlathotep.Tests AuthoringTests Delete (fails when: a first call writes, a confirm deletes after 30 s or from another admin, a running event is deleted, the cooldown row stays, or the confirm deletes a definition other than id)
...
- [ ] D14 · **Pillar commands** `.nyar pillar list` replies one line per pillar, "<name> on|off (Pillars.<Key>)", and a first line "General.Enabled is off: nothing starts" while the master switch is off; `.nyar pillar <name> on|off` sets that ConfigEntry through Services/PillarSwitches over Logic's IPillarStore, saves the cfg, takes effect at once (EventRuntime reads the entries live), and replies "pillar <name> on (saved to cfg)"; setting a pillar to its current value replies "pillar <name> already on" and saves nothing; `off` ends that pillar's running events through the stop path ("event <id> ended (pillar off)", S-7); an unknown name or state replies with the five names or "use on or off" · test: Nyarlathotep.Tests PillarSwitchTests Commands (fails when: a no-op saves, `off` leaves a running event of that pillar or ends another pillar's, an unknown name writes, or the master-switch line is missing while General.Enabled is off)
...
- [ ] D18 · **Static checks and selftest** preflight gains Test-CheckCfgWrites: a write to a [Pillars] ConfigEntry (`.Value =` on EmpowermentEnabled, EventSpawnsEnabled, BossReinforcementsEnabled, DefendedZonesEnabled or SiegeWavesEnabled) or a `ConfigFile.Save` call anywhere but Services/PillarSwitches.cs fails, printing "cfg writes: only PillarSwitches (<n> call sites)"; fixtures CfgWrites/bad (a Commands file setting EventSpawnsEnabled.Value), bad-2 (a Services file calling Config.Save), good (copies of the real files) and empty are registered in tools/preflight-checks.json; TemplatesJson/bad-3 ships templates.json with one template enabled; the soak-report selftest (D24) joins `externalSelfTests` · cmd: pwsh tools/preflight.ps1 -SelfTest → "selftest: <n>/<n> checks, <k>/<k> external selftests"; then pwsh tools/preflight.ps1 → PREFLIGHT OK with "secrets: none", "pillar defaults: all off (5 switches, 2 templates)", "templates: 2 valid" and "cfg writes: only PillarSwitches (<n> call sites)" (fails when: a bad or empty fixture passes, the good one fails, a fixture or the soak selftest is unregistered, a tracked file holds a token shape, or a Resources/*.json ships an enabled event)
- [ ] D19 · **Dependency failures** through Logic's IFileStore and IPillarStore fakes: a failed events.json write replies "could not write events.json: <reason>" and leaves the loaded set and the file unchanged; a cfg save that throws puts the entry back to its old value and replies "could not save the cfg: <reason>; pillar unchanged"; a missing or unparsable catalogue logs "template catalogue unavailable: <reason>" once at boot, every template command replies "template catalogue unavailable", and every other command works · test: Nyarlathotep.Tests DependencyFailureTests Library (fails when: a failed write changes the loaded set, a failed save leaves the new value in memory, a catalogue failure throws, logs more than once, or disables a non-template command)
- [ ] D20 · **Design doc commands** docs/NYARLATHOTEP_DESIGN.md §6 lists `.nyar template list|info|use`, `.nyar event new|copy|delete`, the widened `event set` fields with `location here`, and `.nyar pillar list|<name> on|off`, each as admin; ContractDocTests parses the §6 table and compares it with the command forms the code declares · test: Nyarlathotep.Tests ContractDocTests (fails when: a declared command form is missing from §6, §6 lists a form the code does not declare, or its permission column says anything but admin for a new form)
- [ ] D21 · **Authoring in game** every new command's replies are seen in the real chat (profile note 11.2) and do what D4-D16 say · manual: Session 2 (owner, 127.0.0.1:9876) runs template list, list empowerment, info, use, use as, event new, copy, set of one field of each family including location here at a marked spot, delete then delete confirm, a delete of a running event, pillar list, a pillar on and off, and event list; each reply is recorded verbatim in the feature doc, no reply shows a missing or eaten character, and the spawns event set with location here spawns at the marked spot
...
- [ ] D25 · **Four-hour soak** with every template copied in by `.nyar template use`, enabled, and the empowerment and spawns pillars on, the server runs at least four hours with Debug.TimingLog on · manual: Session 3's logs, archived before each restart, give `pwsh tools/soak-report.ps1 -Log <archived logs> -Templates <the six ids> -MinMinutes 240` → "soak: pass"; its summary line and each template's first start and end lines are copied into the feature doc
...
7. **Readiness (4.4, D16):** a definition's readiness is the first of: invalid, off (pillar), off (event), ready. General.Enabled off is shown once at the top of the list rather than per line. The start precedence of Epic Business rules 1 is unchanged: purge > General.Enabled > the pillar > MaxConcurrentEvents > the definition's own state.
8. **Temporal (4.3):**
...
|---|---|---|---|
| V Rising server (ProjectM) | VampireReferenceAssemblies 1.1.12-r99041-b2; free | unit_index.tsv for every template name (S-4, S-5); the character's Translation for location here; the day and night phase for undead-nightfall (profile note 6.1: Sessions 1 and 3 confirm it fires) | a name the live catalog lacks disables that template at boot with its reason (D3) |
| BepInEx ConfigFile | BepInEx 6.0.0-be.733; free | Save after a ConfigEntry change, other keys and comments kept (S-11, checked by D15) | a throwing Save reverts the entry and replies (D19) |
| VampireCommandFramework (VCF, the chat command library) 0.10.* | pinned with the siblings; free | for example, `.nyar event set x trigger.days Sat,Sun` reaches the method as four string arguments; a quoted argument keeps its spaces | an unparseable argument gets VCF's usage reply; our own shape checks come after (D9, D10) |
...
| git, gh, GitHub; Codex CLI | as faction-empowerment's table | tags, release assets; review prompts | as there: a failed call aborts the step and a rerun finishes it |
...
- **7.2 Concurrency:**
  ...
  - Two admins arming deletes of one id: each arming is keyed by its admin (D8).
...
| Unit tests (D1-D14, D16, D19, D20, D27) | each D-item's fails-when input | the valid cases beside them | a filter running 0 tests fails |
...
| Release, rollback, audit, sessions, paths (D28-D30) | as faction-empowerment's matrix, at v0.5.0 | as there | as there |
...
| 4.4 precedence | D16 `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter FullyQualifiedName~ReadinessTests` | a lower readiness winning over a higher one | each single cause | 0 cases → failure |
| 6.2 dependencies | D19 `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter FullyQualifiedName~DependencyFailureTests` | a failure changing memory or escaping its command | healthy fakes | 0 cases → failure |
...
| 12.4 failing cases | D18 `pwsh tools/preflight.ps1 -SelfTest`, which runs D24's soak selftest as a registered external selftest | a bad or empty fixture passing | good fixtures | an empty registry → failure |
...
| 1 MB events.json (foundation MaxFileBytes) | foundation's validator | a write that would exceed it is not written (D12) | very long announce pools across 200 events |
...
1. **Records and pure logic.**
   - Create docs/audits/event-library.md (rollback base v0.4.0) and docs/features/EVENT_LIBRARY.md with Status, Test plan and an empty Test results.
   ...
   - Tests: TemplateLibraryTests, TemplateCommandTests, AuthoringTests, AuthoringCapacityTests, PillarSwitchTests, ReadinessTests, DependencyFailureTests.Library.cs, and additions to CommandArgTests, ConfigChangedTests and AuthorizationTests; FakeStores gains a fake IPillarStore.
   - Run `dotnet test Nyarlathotep/Nyarlathotep.Tests`.
   - Satisfies D1, D2, D4, D5, D6, D7, D8, D9, D10, D11, D12, D13, D14, D16, D19, D27.
2. **Services, commands, checks and docs.**
   - New Services/TemplateLibrary.cs (boot load, "templates: <v>/<n> valid") and Services/PillarSwitches.cs (ConfigEntry set, ConfigFile Save, revert on throw), both [Mutating] behind Gateway.Run.
...
3. **Unattended session (Session 1) and the soak tool.**
   - Write tools/soak-report.ps1 with its -SelfTest and fixtures under tools/soak-report-fixtures/ (D24), register it in `externalSelfTests`, and add the `temp:` glob %TEMP%\nyar-soak-* to tools/paths-manifest.txt and its entry to tools/data-inventory.json.
...
5. **Four-hour soak (Session 3).**
   - Add mode `soak` to tools/ingame/session-events.py: copies of the six templates, all enabled; legion-weekend-surge's copy retimed to every day at twelve times spread over the soak window; the empowerment and spawns pillars and Debug.TimingLog on.
   - Run `pwsh tools/dev-snapshot.ps1 -Save s3`, deploy, and boot.
   - Owner kick-off (S-13): the owner kills one bandit V Blood and one Militia or Church V Blood on the list of Business rules 2, and runs `.nyar event start bandit-ambush` and `.nyar event start undead-rising`; then the owner may leave.
...
- W4 · **Controls**
- W4.1 · **Auth, checks, failures, docs** · items: D17 D18 D19 D20 · steps: 1, 2
...
- **Step 1:**
  ...
  - Nyarlathotep/Nyarlathotep.Tests/{TemplateLibraryTests,TemplateCommandTests,AuthoringTests,AuthoringCapacityTests,PillarSwitchTests,ReadinessTests,DependencyFailureTests.Library,CommandArgTests,ConfigChangedTests,AuthorizationTests,FakeStores}.cs.
- **Step 2:**
...
- S-7 · reversible · `.nyar pillar <name> off` ends that pillar's running events at once, like stop · fallback: leave them running to their end and only block new starts, a change to one planner in Logic/Pillars.cs and its test
- S-8 · reversible · the scope's "counts" are the `:<count>` suffixes of action.units, so one field sets units and counts together · fallback: add action.units.<n>.count as its own field, one more CommandArgs row and test
...
- S-11 · reversible · ConfigFile.Save rewrites the cfg keeping every other key's value and the comments BepInEx writes · fallback: if D15's diff shows more than one changed line, PillarSwitches edits the one line itself through Persistence, with its own test
...
- S-13 · reversible · the soak runs unattended after an owner kick-off of a few minutes, because two templates need a real V Blood kill and two need a Manual start at an admin's position · fallback: if the owner cannot kick off, the soak is split into an unattended part (the Schedule and GameTime templates) and the V Blood and Manual templates are proved in Session 2 instead, recorded as an amendment of D25
...
| 4 | Business rules & invariants | Considered | 5/5 | Business rules › 4.1 D2 D8 D16; 4.2 D1 D12 D5; 4.3 D8 D14; 4.4 D16 D14; 4.5 D1 D13 D17 D30 D20 |
| 5 | Internal interfaces | Considered | 3/3 | Interfaces › 5.1 D3 D12; 5.2 D12 D14; 5.3 D1 D10 D20 |
| 6 | External dependencies & contracts | Considered | 3/3 | Interfaces › 6.1 D3 D15 D25; 6.2 D19; 6.3 D24 D30 |
...
| 12 | Failure handling & observability | Considered | 4/4 | Failure & observability › 12.1 D19 D21; 12.2 D3 D14 D25; 12.3 D16 D25 D30; 12.4 D18 D24 |
