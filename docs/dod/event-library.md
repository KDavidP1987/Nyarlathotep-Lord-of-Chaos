...
- **12.1:**
  ...
  - A write or save failure names the file and leaves memory unchanged (D19); the one exception is a delete's second write (state.json), where the definition stays deleted and the reply says the cooldown row is cleared on the next save (D8, D19).
...
| 6.2 dependencies | in game: D19 `dotnet test Nyarlathotep/Nyarlathotep.Tests --filter FullyQualifiedName~DependencyFailureTests`; release time: D18 `pwsh tools/preflight.ps1 -SelfTest`, which re-runs release-verify's and repo-rollback-drill's failure cases | a failure changing memory or escaping its command; a delete whose state.json write fails restoring the definition or keeping the row past the next save; a release-tool failure case printing its success line | healthy fakes; the good cases | 0 cases → failure; an empty registry → failure |
...
1. **Records and pure logic.**
   ...
   - Tests: TemplateLibraryTests, TemplateCommandTests, AuthoringTests, AuthoringCapacityTests, PillarSwitchTests, ReadinessTests, LibraryDependencyFailureTests (in DependencyFailureTests.Library.cs), ControlCaseTests, and additions to CommandArgTests, ConfigChangedTests, AuthorizationTests, ContractDocTests and ControlPrecedenceTests, every new method named by D31's convention; LibraryDependencyFailureTests gains the delete whose state.json write fails; FakeStores gains a fake IPillarStore and a state store that fails its next write.
...
- **Step 1:**
  ...
  - Nyarlathotep/Nyarlathotep.Tests/{TemplateLibraryTests,TemplateCommandTests,AuthoringTests,AuthoringCapacityTests,PillarSwitchTests,ReadinessTests,DependencyFailureTests.Library,ControlCaseTests,CommandArgTests,ConfigChangedTests,AuthorizationTests,ContractDocTests,ControlPrecedenceTests,FakeStores}.cs.
- **Step 2:**
