...
4. S1 march (Spikes/SpikeMarch.cs). `.nyar spike march <v> [count] [distance]` spawns `count` CHAR_Bandit_Thug (-301730941, S-5) `distance` m north of the admin. The destination is the admin's position when the command runs.
   ...
   - Variant 4 probe (A6): each second the mover logs the first unit's live AggroConsumer ProximityRadius, MaxDistanceFromPreCombatPosition and Active, AggroModifiers CircleRadiusFactor and ConeRadiusFactor, the AggroBuffer entry count and whether the admin is still in it; then it re-applies the hunt (one AggroBuffer entry per target) to every unit not yet in Combat.
