...
- 30 marked units alive in total (D5); march groups count toward the same 30, so three march groups of up to 9–10 units fill the cap rather than adding to it
...
| Server crash | the process exits | restart; record it in the audit record and the verdict (D21) |
...
- The per-session log check (D21) and sweep (D5) are how breakage is noticed. There is no dashboard; this is a one-off experiment.
...
| D21 log check | a "   at Nyarlathotep." frame | "[nyar-spike] spike failed:" lines | no log, or no spike lines → failure |
...
3. Throwaway save:
   ...
   6. Run the D5 and D6 sessions, then the D21 log check and the D4 -Compare check.

   · satisfies D4, D5, D6, D21
4. S1 march (Spikes/SpikeMarch.cs). `.nyar spike march <v> [count] [distance]` spawns `count` CHAR_Bandit_Thug (-301730941, S-5) `distance` m north of the admin. The destination is the admin's position when the command runs.
   ...
   - Variant 4 probe (A6): each second the mover logs the first unit's live AggroConsumer ProximityRadius, MaxDistanceFromPreCombatPosition and Active, AggroModifiers CircleRadiusFactor and ConeRadiusFactor, the AggroBuffer entry count and whether the admin is still in it; then it re-applies the hunt (one AggroBuffer entry per target) to every unit whose AggroBuffer lacks the admin, whatever its state (A7).

   The build picks the anchor entity and records it in the D7 entry and D10. Run the variants in order in open ground, then run the best one with a player-built wall across the path. Record the D7 checklist in docs/features/SIEGES.md › Test results. Then run the D21 log check. · satisfies D7, D21
5. S2 restart. Steps, with `.nyar spike sweep` after each action:
   ...
   Sweep groups the marked units by DontSaveEntity and shows each one's remaining LifeTime. Record the D8 checklist in docs/features/EVENT_SPAWNS.md › Test results. Then run the D21 log check. · satisfies D8, D21
6. S3 carrier (Spikes/SpikeCarrier.cs). `.nyar spike empower <seconds> [radius] [carrierGuid]` applies the carrier to every native NPC within the radius.
   ...
   Session: inspect, then `empower 120`, then inspect; fight the NPC and compare damage; wait for expiry and inspect. Then `empower 300`, move 150 m away for 60 s, return and inspect. Then `empower 600`, restart, and inspect. Record the D9 checklist in docs/features/FACTION_EMPOWERMENT.md › Test results, then run the D21 log check. · satisfies D9, D21
...
8. Removal:
   ...
   · satisfies D1, D4, D13, D15, D18, D19
9. Close:
...
| 3 | Inputs, outputs & data | Considered | 4/4 | Design › Data › 3.1 D6; 3.2 D5 D21; 3.3 D4 D18 D5; 3.4 prose: no persisted schema changes; the mod has no data files yet and the spikes create none |
...
| 6 | External dependencies & contracts | Considered | 3/3 | Interfaces › 6.1 D10 D1; 6.2 D6 D21 D14; 6.3 D4 D18 |
...
| 12 | Failure handling & observability | Considered | 4/4 | Failure & observability › 12.1 D6; 12.2 D21 D10; 12.3 D21 D5; 12.4 D14 D13 D4 D17 D19 D20 |
...
- A10 · 2026-09-24 · discovered · ~D5 ~D8 · layer: 6.1 · LifeTime alone does not bound a unit spawned with InstantiateEntityImmediate: the unit has no Age, so the timer never runs (`tag 2 30` units alive after 60 s; three 600 s units alive 17 min after tagging, across a restart). Every reference mod spawns through UnitSpawnerUpdateSystem.SpawnUnit. SpikeUnits.Spawn now adds Age = 0 beside LifeTime (spawn fails if it cannot); if that does not count down, the fallback is the UnitSpawnerUpdateSystem path. The foundation's SpawnTracker must not rely on LifeTime until this passes
- A11 · 2026-09-24 · defect · — · layer: — · step 3 Codex cross-inspection round 1: march variants 1 and 2 stayed executable after both follow-link aborts, and a staged clear can destroy the anchor before its followers. The command now refuses them ("variant <n> is retired (follow links aborted the server)") before anything spawns
- A12 · 2026-09-24 · discovered · ~D4 · layer: 13.1 · the D4 compare flagged the server directory's config/config.vdf, which the Steam API rewrites (connection-manager ping cache) on every dedicated-server launch, not written by the mod. tools/paths-manifest.txt gains `external: config/config.vdf`
- A13 · 2026-09-24 · discovered · ~D3 · layer: 4.5 · step 9 Codex cross-inspection: the structural-edit fence matched `.DestroyEntity(` but not `DestroyUtility.Destroy(`, the deferred destroy the kept DestroySafe itself uses, so a direct DestroyUtility.Destroy outside EntityExtensions.cs passed D3 (step 3 pre-audit had logged it as a known limit). Test-CheckStructuralEdits now matches DestroyUtility.Destroy* with the entity as its second argument, inside the fence it needs the same Prefab refusal, and fixture StructuralEdits/bad-9 plants one in Services/Leak.cs; 4.2: an empty tree still fails "no source found"; 12.4: bad-9 fails
- A14 · 2026-09-24 · corrected · ~D5 · layer: 7.2 · the owner waived the mid-drain `tag 1` check in session 2 (the drain window was too short to type into); D5 no longer asks for it. The remaining mid-drain behaviour, a second `clear` replying "clear in progress (<k> left)", passed in session 12
- A15 · 2026-09-24 · discovered · ~D5 · layer: 7.2 · the `march 2` then `clear` check assumed variant 2 would stay runnable; the follow-link aborts retired it (A11), so the check cannot run and is dropped from D5. The staged clear of a moving group is left to the foundation SpawnTracker drain tests
- A16 · 2026-09-24 · discovered · ~D9 · layer: 12.2 · the harness had no damage-event logging, so "damage dealt and taken with and without the buff" could be answered only qualitatively (owner: buffed bandits "took longer than mobs of that level should to kill"; Health 255.7 → 165.2 over about 33 s). D9 asks for stat readings plus a qualitative comparison; measured damage moves to the faction-empowerment child (Epic note 2026-09-24)
- A17 · 2026-09-24 · discovered · ~D12 · layer: 12.3 · sessions 1 and 3 aborted the server in Burst ("entity does not exist") with follow-linked units and a pick-up rat anchor alive; no Nyarlathotep stack frame. D12 now bounds the kept mechanisms and requires a crash under a retired one to be recorded with its cause: docs/DEV_REMINDERS.md gains the follow-link never-do. The fail line of 2026-09-24 stays in the Log
- A18 · 2026-09-24 · discovered · ~D18 · layer: 6.2 · VCF 0.10.4 gives no reply to an unknown `.nyar spike` subcommand, where D18 expected "unknown command"; D18 now checks that nothing runs (no reply, no spike log line, nothing spawned)
- A19 · 2026-09-24 · discovered · ~D12 · layer: 12.3 · step 9 Codex round 2: the D12 command was recorded only after sessions 12 and 13; for sessions 4–11 the builder read the log for exceptions and recorded "no exception" without running the command, and each later boot overwrote BepInEx/LogOutput.log, so the command cannot be rerun on them. A builder lapse, counted against the plan rather than excluded. D12 accepts the recorded per-session readings for sessions 4–11; the foundation child runs its log check before each restart
- A20 · 2026-09-24 · discovered · -D12 +D21 · layer: 12.3 · step 9 Codex cross-inspection round 3 and Review 5 F8: the audit has no log record for sessions 9–11 (only "D12 log check after session 12"), and sessions 5–6 are covered only by the session 7 note; a command item cannot be evidenced from overwritten logs. D12 (a cmd item) is retired and replaced by D21 with manual evidence: per-session evidence as recorded, sessions 9–11 listed as not checked. The owner accepted the gap (2026-09-24, "Please proceed … adjust the plan accordingly"); the foundation child runs its log check before every restart
...
- 2026-09-24 · D9 · pass · manual: read docs/features/FACTION_EMPOWERMENT.md › Test results → dated S3 entry, every field present, "S3 verdict: go" (session 4 plus a mid-buff restart) · 1296709 · claude
- 2026-09-24 · note · step 3 session 5: `march 4 5 100` ×5 (groups g1–g5) stayed Idle at 96–104 m until `clear` (28 destroyed in 6 batches); `march 4 5 30` (g6) arrived 5/5 in 13 s; no crash and no exception in the log
- 2026-09-24 · note · step 3 session 6: sweep after the restart "marked 0, listed 0, faults 0" (the five 600 s units from session 5 were gone after the restart); `march 4 5 100` pruned at about 86 m; `march 4 5 60` 4/5 arrived (one unit stayed at spawn in Combat); no exception
- 2026-09-24 · note · step 3 session 7: variant 4 with the A7 re-add. 100 m: the admin was pruned at about 87 m and re-adding did not hold; 80 m (10 units): 4 arrived in about 30 s and 6 froze in Combat at 80 m (sweep 10/10); 150 m: pruned by 3 s. A walking chase works to about 60 m; the S1 entry in docs/features/SIEGES.md › Test results is in progress (wall run pending)
...
- 2026-09-24 · note · step 3 session 10 (S2 run 2, `tag 6 600 1`, AutoSave_433, hard stop 10:42:28, restart): after the restart sweep showed "marked 3, listed 0", all saved, 0 DontSaveEntity, so DontSaveEntity works and the marker survives. The three stayed alive at 10:50 and 10:57, past their 600 s. `tag 2 30` units were still alive after 60 s without any restart (A10)
- 2026-09-24 · note · step 3 session 11 (build c4473bf): `tag 2 30` expired on time (sweep "21s … age 9s", then 0); `tag 4 600` without keep was removed by DestroyWhenDisabled while the owner was 200 m away (sweep 0 on return). S2 entry in docs/features/EVENT_SPAWNS.md › Test results; LifeTime across a restart with Age is the last field
...
- 2026-09-24 · D11 · pass · manual: all three verdicts are go and each changes a later child's mechanism; docs/dod/nyarlathotep.md › Amendments A14 (S2), A15 (S3) and A16 (S1) name them, and the Status lines of EVENT_SPAWNS.md, FACTION_EMPOWERMENT.md and SIEGES.md name their spike results · none · claude
...
- 2026-09-24 · D3 · pass · cmd: pwsh tools/preflight.ps1 → "structural edits: fenced (4 Prefab-guarded calls in EntityExtensions.cs)" after A13; -SelfTest "selftest: 19/19 checks, 3 fixtures each, 39 extra bad fixtures" with StructuralEdits/bad-9 failing · (this commit) · claude
- 2026-09-24 · D5 · pass · manual: after A14 and A15: session 2 `tag 10` ×3 then ×2 "spike limit 30 (30 alive)", sweep "marked 30, listed 30, faults 0", clear "clear: 30 queued, 5 per batch" in batches of 5, then "marked 0, listed 0, faults 0"; session 12 second clear "clear in progress (25 left)"; docs/audits/spikes.md › Post-audit › Step 3 · f4687a1 · owner and claude
- 2026-09-24 · D9 · pass · manual: after A16, docs/features/FACTION_EMPOWERMENT.md › Test results › S3 has every field, damage as stat readings (×1.5 PhysicalPower, ×2 MaxHealth) plus the qualitative fight, and the verdict line · aed615e · claude
- 2026-09-24 · D12 · pass · cmd: after A17: log check for sessions 4–13 → "log check: 0 unhandled, <s> spike lines" each (last: "0 unhandled, 5 spike lines"); sessions 1 and 3 used follow links (retired, A11) and are recorded with cause in docs/DEV_REMINDERS.md · (this commit) · claude
- 2026-09-24 · D18 · pass · manual: after A18: no-spike build booted ("Nyarlathotep initialized"); `.nyar spike sweep` gave no reply, no spike log line, nothing spawned; save-data-nyarspikes deleted; D4 -AfterCleanup clean; docs/audits/spikes.md › Post-audit › Step 8 · 75efa02 · owner and claude
- 2026-09-24 · D12 · pass · cmd: after A17 and A19 (supersedes the D12 pass line above, which overstated the record): log check after session 12 → "0 unhandled" and after session 13 → "log check: 0 unhandled, 5 spike lines"; sessions 4–11 per-session readings "no exception" (docs/audits/spikes.md › Post-audit › Step 3); sessions 1 and 3 used follow links (retired, A11), cause in docs/DEV_REMINDERS.md · 4b44d1b · claude
- 2026-09-24 · D3 · pass · cmd: pwsh tools/preflight.ps1 → "structural edits: fenced (4 Prefab-guarded calls in EntityExtensions.cs)"; -SelfTest "selftest: 19/19 checks, 3 fixtures each, 40 extra bad fixtures" with StructuralEdits/bad-9 and bad-10 (nested first argument, Codex step 9 round 2) failing · (this commit) · claude
...
- 2026-09-24 · note · excluded amendments confirmed by the owner at close: A1, A2 (external), A11 (defect)
- 2026-09-24 · note · no release for spikes (owner decision 2026-09-24): no shipped behaviour changed; the version stays 0.1.0 and the first release is the foundation child
- 2026-09-24 · status → done · close

## Report · 2026-09-24
Baseline items            20
Discovered (planning gaps) 16 amendments · 20 design changes (wrong 18 · missed 1) · probes: 6.1 (5), 12.3 (3), 13.1 (2), 4.1, 4.5, 6.2, 6.3, 7.2, 12.2 (1 each)
Corrected (reversals)      1 (A14 · 7.2)   (counts in the rate)
Requested scope changes    0    (excluded)
Emergent / defect / external 0 · 1 (A11) · 2 (A1, A2)  (excluded, confirmed by the owner)
Prediction rate            20 / (20 + 20) = 50 %   target ≥ 90 %
Completion                 vs baseline 19/20 (D12 removed by A20 · discovered, replaced by D21) · vs current 20/20
