...
9. **Precedence (4.4):** push volume yields to the queue caps (50 lines, 5 lines per tick), which yield to nothing: a dropped line is logged, never retried. A player's subscription never overrides the S-1 gates. The owner decides exceptions by changing the switches.
...
## Performance
- **Budget (13.1):** the scheduler tick keeps foundation D24's averages, under 5 ms at 150 tracked units, with pushes on (D22). Pusher.Tick is the new hot path. With an empty queue it is one count check. Otherwise it sends at most 5 lines to each subscriber, at most 640 sends a tick; D22 records the per-tick time with a live subscriber and PushTests pins the 640-send ceiling. An api read replies within one second in chat (D22); a read is O(active events + cleanups + tracked units), at most 10 + 10 + 500.
- **Bounds (13.2):**
