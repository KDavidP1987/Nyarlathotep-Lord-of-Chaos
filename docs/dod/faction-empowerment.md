...
- [ ] D28 · **Dev-server snapshot** (profile note 14.3) tools/dev-snapshot.ps1 wraps every session of steps 4-6: `-Save <label>` refuses while a VRisingServer process runs or while a %TEMP%\nyar-snap-* folder holding a manifest exists (printing its recorded restore), then copies BepInEx/plugins, BepInEx/config/kdpen.Nyarlathotep.cfg and BepInEx/config/Nyarlathotep/ with hidden files, and writes saved\manifest.json (each path's presence and SHA-256) through a .tmp and a move, last; `-Restore` puts every path back or removes it when it was absent, verifies every hash, and deletes the folder only when all match; the snapshot and restore functions are shared with tools/rollback-drill.ps1 (moved into tools/snapshot-lib.ps1) · cmd: pwsh tools/dev-snapshot.ps1 -SelfTest → "snapshot selftest: 6/6" (a scratch install: save and restore round-trip with a hidden file, a leftover manifest refused, a partial snapshot without its manifest removed, a restore whose copy is corrupted keeps the folder, an absent config folder restored as absent, a running-server refusal); then each session in docs/audits/faction-empowerment.md records "snapshot restored; hashes equal" (fails when: a mutated restore reports equal hashes, a leftover manifest is overwritten, an absent path comes back present, or a session lacks the restored line)
...
- **On the dev server during the build:** every session is wrapped by tools/dev-snapshot.ps1 (D28), so the plugins folder, the cfg and the mod's config folder return to their pre-session state, hash-verified.
...
- S-8 · reversible · Bloodcraft familiars carry a Follower, EntityOwner or Team that leads to their player, so Ownership.Decide skips them · fallback: Session 3 records the familiar's components; if none leads to the player, the component Bloodcraft does set is added to Ownership.Decide by a `discovered` amendment before release, and until then the familiar's faction can be kept out of events
