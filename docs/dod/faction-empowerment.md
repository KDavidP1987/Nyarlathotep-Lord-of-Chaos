...
- [ ] D22 · **Static checks and selftest** preflight prints PREFLIGHT OK with "secrets: none", "pillar defaults: all off", "structural edits: fenced", "patch guards: <n>/<n>", "gateway: only ActionGateway mutates (<n> call sites)" and "wire contract: <n> tags, <m> api commands, all documented (api 3)"; tools/preflight-checks.json registers StructuralEdits/bad-2 (a real Services file with a planted DestroyUtility.Destroy), StructuralEdits/bad-3 (the real EntityExtensions.cs with the call moved into DestroySafe) and TemplatesJson/bad-2 (the real template with example-empowerment enabled), and -SelfTest passes, including the existing Secrets bad fixtures (a planted tss_ token and a tools/ script calling `gh auth token`) · cmd: pwsh tools/preflight.ps1 -SelfTest → "selftest: <n>/<n> checks" with no failing fixture line; then pwsh tools/preflight.ps1 → PREFLIGHT OK with those lines (fails when: a bad-n or empty fixture passes, a good one fails, a new fixture is unregistered, or a tracked file holds a token shape)
...
| -AuditOf, -SessionsOf, -Paths, data inventory (D26) | their existing bad fixtures (a step without a verdict, a session without its line, an unlisted path, an entry missing a field) and Paths bad-temp, bad-worktree, bad-remote (recorded listings with a leftover folder, a second worktree, an undeclared tag) | copies of the real files | existing empty fixtures, each printing a failure line |
...
| 14.4 paths | D26 `pwsh tools/preflight.ps1 -Paths` | a walked path matching no glob, a leftover %TEMP% snapshot or rollback folder, a leftover worktree, or an undeclared remote tag or release | manifested paths | an empty manifest → failure |
...
7. **Release.**
   - Extend tools/rollback-drill.ps1 and its fixtures (D23); save D25's command as tools/repo-rollback-drill.ps1 and write tools/rollback-gate.ps1 with its -SelfTest (D29).
...
- **Step 2:**
  ...
  - tools/preflight.ps1, tools/preflight-checks.json, tools/preflight-fixtures/StructuralEdits/bad-2/**, tools/preflight-fixtures/StructuralEdits/bad-3/**, tools/preflight-fixtures/TemplatesJson/bad-2/**, tools/preflight-fixtures/Paths/{bad-temp,bad-worktree,bad-remote}/**.
...
- **Step 7:**
  - tools/rollback-drill.ps1, tools/rollback-drill-fixtures/**, tools/repo-rollback-drill.ps1, tools/rollback-gate.ps1, tools/paths-manifest.txt.
