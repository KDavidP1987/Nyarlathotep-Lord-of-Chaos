...
- [ ] D14 · **Debug shows natives** `.nyar debug here [radius]` (admin) also lists, after the tracked units, at most 10 native NPCs within the radius, nearest first, one line each: "<prefab> native <faction short> d <m>m carrier <id> left <n>s type <BuffType> stacks <MaxStacks> end <EndAction> mark <ok|bad> lvl L hp cur/max pp P sp S aspd A mspd M", read back from the live carrier entity, with "carrier none" in place of the carrier fields for a plain NPC; the selection and the line are built by Logic AdminLines.Natives(rows, 10) · test: Nyarlathotep.Tests AdminLinesTests (fails when: 11 rows give more than 10 lines, the lines are not in non-decreasing distance, a tie is not broken by prefab then key, a carried row lacks its id or seconds, or a plain row shows other than "carrier none"); manual: Session 2 records the rows at a bandit camp with more than 10 natives in range
...
| 14.3 rollback | D25 repository drill, D23 `pwsh tools/rollback-drill.ps1 -From v0.4.0 -To v0.3.0`, and D28 `pwsh tools/dev-snapshot.ps1 -SelfTest` for the live dev install | a conflicting revert; N-1 failing on N's files or disabling for another reason; a mutated restore passing | a clean range; a faithful restore | fewer than two release tags → "rollback drill: needs two releases"; no scratch install → "snapshot selftest: 0/6", a failure |
...
2. **Service, template and checks.**
   ...
   - tools/preflight.ps1: the structural-edits check allows DestroyUtility.Destroy only inside RemoveBuffSafe's body; fixtures StructuralEdits/bad-2, StructuralEdits/bad-3 and TemplatesJson/bad-2 registered in tools/preflight-checks.json.
...
- **Step 2:**
  ...
  - tools/preflight.ps1, tools/preflight-checks.json, tools/preflight-fixtures/StructuralEdits/bad-2/**, tools/preflight-fixtures/StructuralEdits/bad-3/**, tools/preflight-fixtures/TemplatesJson/bad-2/**.
...
| 14 | Rollout & compatibility | Considered | 4/4 | Rollout › 14.1 D24 D22; 14.2 D2 D11 D27; 14.3 D25 D23 D28; 14.4 D26 |
